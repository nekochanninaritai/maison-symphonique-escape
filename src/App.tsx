...
import photoEImage from './assets/environments/photoE.jpg'
...
import hourHandItemImage from './assets/environments/item/hour-hand.png'
...
import bgmChapter4 from './assets/audio/4章.mp3'
import bgmEnding from './assets/audio/エンディング.mp3'

const bgmByArea: Record<AreaId, string> = {
  'entrance': bgmChapter1,
  'waiting-room': bgmChapter2,
  'dressing-room': bgmChapter1,
  'ceremony': bgmChapter3,
  'reception': bgmChapter3,
  'garden': bgmChapter4,
}
...
function App() {
  ...
  useEffect(() => {
    ...
    if (state.screen === 'game') {
      nextBgm = bgmByArea[state.currentArea]
    } else if (state.screen === 'normalEnd' || state.screen === 'photoE' || state.screen === 'trueFade') {
      nextBgm = bgmChapter4
    } else if (state.screen === 'trueEnd') {
      nextBgm = bgmEnding
    }
    ...
    currentAudio.volume = audioSettings.bgmVolume
  ...
  return (
    <main className="appShell">
      ...
      {state.screen === 'trueFade' && <TrueEndFade onComplete={() => send({ type: 'GO_TRUE_END' })} />}
    ...
    <section className="gameScreen">
      ...
      <div className="stagePanel">
        <div
          ...
          onClick={(event) => {
            if (state.messageQueue.length === 0 || event.target !== event.currentTarget) return
            onAction({ type: 'CLEAR_MESSAGES' })
          }}
        >
          ...
          {stageHotspots.map((hotspot) => (
            <button
              ...
              onClick={() => {
                ...
                if (hotspot.id === 'garden-true-light') {
                  onAction({ type: 'OPEN_TRUE_GARDEN_LIGHT' })
                  return
                }
          ...
          {state.messageQueue.length > 0 && (
            <div className="stageMessageOverlay" aria-live="polite" aria-label="探索メッセージ">
              <div
                className="stageMessageBox"
                role="status"
                onClick={(event) => {
              ...
              >
                {state.messageQueue.map((message, index) => message ? <p key={`${index}-${message}`}>{message}</p> : null)}
              </div>
            </div>
          )}
        </div>
      ...
      <section className="bottomConsole" aria-label="探索情報">
        <Inventory state={state} onInspectItem={onItemFocus} onAction={onAction} />
...
function ItemFocus({ item }: { item?: Item }) {
  ...
  return (
    <div className={`clockWidget ${timeChanged ? 'timeChanged' : ''}`}>
      <button
        ...
        aria-label="大時計"
      >
        {state.clockState.handAttached && <span className="clockHand hour" style={{ transform: `rotate(${hourDeg}deg)` }} />}
  ...
  return (
    <div className="grandClockFocus">
      ...
      <div className={`largeClockFace ${canManipulate ? 'manual' : ''} ${state.clockState.handAttached ? 'handAttached' : 'handMissing'}`} role="group" aria-label="大時計">
        {state.clockState.handAttached && <span className={`clockHand hour large ${activeHand === 'hour' ? 'active' : ''}`} style={{ transform: `rotate(${hourDeg}deg)` }} />}
        ...
        {!state.clockState.handAttached && <span className="missingHand">長針・短針なし</span>}
      </div>
        ...
        {obtainedItems.map((item) => (
          <div key={item.itemId} className="inventoryItem">
            <button
              ...
              title={item.name}
              onClick={() => {
                if (item.usableTargets.length > 0) {
                  onAction({ type: 'SELECT_ITEM', itemId: item.itemId })
                }
                onInspectItem(`item:${item.itemId}`)
              }}
            >
...
function TrueEndFade({ onComplete }: { onComplete: () => void }) {
  useEffect(() => {
  ...
  return (
    <section className="trueEndFade" aria-label="TRUE ENDへ進む光">
      <div className="trueEndFadeGlow" aria-hidden="true" />
    </section>
  )
}
...
function DebugPanel({ state, showHotspots, onToggleHotspots, onAction }: { state: GameState; showHotspots: boolean; onToggleHotspots: () => void; onAction: (action: GameAction) => void }) {
  ...
  const inventoryDebugIds = ['clock-hand', 'clock-hour-hand', 'transparent-card', 'small-key', 'old-invitation', 'letter']
  ...
  return (
    <aside className="debugPanel">
      ...
      <DebugGroup title="Item">
        ...
        <button type="button" onClick={() => onAction({ type: 'OBTAIN_ITEM', itemId: 'clock-hour-hand' })}>短針取得</button>
