...
describe('ClockState', () => {
  it('starts with both clock hands missing', () => {
    const state = createInitialState()

    expect(state.inventory['clock-hand'].obtained).toBe(false)
    expect(state.inventory['clock-hour-hand'].obtained).toBe(false)
    expect(state.clockState.handObtained).toBe(false)
    expect(state.clockState.hourHandObtained).toBe(false)
    expect(state.clockState.handAttached).toBe(false)
  })

  it('attaches both hands but does not start the clock', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'OBTAIN_ITEM', itemId: 'clock-hand' })
    state = reducer(state, { type: 'OBTAIN_ITEM', itemId: 'clock-hour-hand' })
...
describe('PuzzleState', () => {
  ...
  it('attaching both clock hands before P02 does not start the Grand Clock', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'SOLVE_PUZZLE', puzzleId: 'p01_waiting_room' })
    state = reducer(state, { type: 'OBTAIN_ITEM', itemId: 'clock-hand' })
    state = reducer(state, { type: 'OBTAIN_ITEM', itemId: 'clock-hour-hand' })
  ...
  it('minute hand alone does not satisfy the Grand Clock requirement', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'OBTAIN_ITEM', itemId: 'clock-hand' })
    state = reducer(state, { type: 'SELECT_ITEM', itemId: 'clock-hand' })
    state = reducer(state, { type: 'USE_SELECTED_ITEM', targetId: 'grand-clock' })
  ...
  it('hour hand alone does not satisfy the Grand Clock requirement', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'OBTAIN_ITEM', itemId: 'clock-hour-hand' })
    state = reducer(state, { type: 'SELECT_ITEM', itemId: 'clock-hour-hand' })
    state = reducer(state, { type: 'USE_SELECTED_ITEM', targetId: 'grand-clock' })

    expect(state.clockState.handAttached).toBe(false)
    expect(state.inventory['clock-hour-hand'].obtained).toBe(true)
    expect(canManuallyControlGrandClock(state)).toBe(false)
    expect(state.clockState.currentTime).toBe('11:00')
  })
  ...
  it('selected transparent sheet on the unfinished picture applies the overlay and solves P04', () => {
    ...
    expect(state.inventory['transparent-card'].obtained).toBe(false)
    expect(state.inventory['transparent-card'].consumed).toBe(true)
  ...
  it('correct P05 phrase solves the puzzle and shows the bell event once', () => {
    ...
    expect(solvedState.flags.invitationObtained).not.toBe(true)
    expect(solvedState.inventory['old-invitation'].obtained).toBe(false)
    expect(solvedState.inventory['clock-hour-hand'].obtained).toBe(false)
    expect(solvedState.messageQueue).toEqual([
      '最後の音が、静かな披露宴会場に響いた。',
      '――そのとき、館のどこかから、小さな鐘の音が響いた。',
    ])
  ...
  it('P05 solved unlocks the piano mechanism but keeps the invitation and hour hand in the piano secret', () => {
    let state = createInitialState()
    state = solvePuzzle(state, 'p05_piano', true)

    expect(state.flags.pianoMechanismUnlocked).toBe(true)
    expect(state.flags.invitationObtained).not.toBe(true)
    expect(state.inventory['old-invitation'].obtained).toBe(false)
    expect(state.inventory['clock-hour-hand'].obtained).toBe(false)
    expect(state.puzzles.p06_grand_clock.status).toBe('locked')
  })

  it('P06 becomes available after opening the piano secret but clock operation waits for both hands', () => {
    let state = createInitialState()
    state = solvePuzzle(state, 'p05_piano', true)
    ...
    state = reducer(state, {
      type: 'USE_SELECTED_ITEM',
      targetId: 'piano-keyhole',
    })
  ...
  it('NORMAL END continue grants the letter hint item', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'GO_NORMAL_END' })

    expect(state.inventory.letter.obtained).toBe(false)

    state = reducer(state, { type: 'START_GAME' })

    expect(state.inventory.letter.obtained).toBe(true)
    expect(state.messageQueue).toEqual(['古い手紙を手に入れた。'])
  })

  it('09:23 after NORMAL END hears music from the Garden without opening TRUE END', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'ATTACH_CLOCK_HAND' })
    state = reducer(state, { type: 'GO_NORMAL_END' })
    state = reducer(state, { type: 'START_GAME' })
    ...
    expect(state.clockState.currentTime).toBe('09:23')
    expect(state.flags.trueGardenMusicHeard).toBe(true)
    expect(state.trueRouteUnlocked).toBe(false)
    expect(state.memories.september23.unlocked).toBe(false)
    expect(state.screen).toBe('game')
    expect(getMemoryCount(state)).toBe(4)
    expect(state.messageQueue).toEqual(['――庭園の方から、かすかに音楽が聞こえる。'])
  })

  it('09:23 does not trigger the Garden music without the letter', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'ATTACH_CLOCK_HAND' })
    state = reducer(state, { type: 'GO_NORMAL_END' })
    state = {
      ...
      inventory: {
        ...state.inventory,
        letter: { ...state.inventory.letter, obtained: false, consumed: false },
      },
    }
    state = reducer(state, { type: 'EXAMINE', hotspotId: 'grand-clock' })
    state = reducer(state, { type: 'SET_CLOCK_TIME', time: '09:23' })

    expect(state.flags.trueGardenMusicHeard).not.toBe(true)
    expect(state.trueRouteUnlocked).toBe(false)
    expect(state.memories.september23.unlocked).toBe(false)
  })

  it('Garden light appears after 09:23 music and starts the TRUE END fade', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'ATTACH_CLOCK_HAND' })
    state = reducer(state, { type: 'GO_NORMAL_END' })
    state = reducer(state, { type: 'START_GAME' })
    state = reducer(state, { type: 'SET_FLAG', flagId: 'gardenUnlocked', value: true })
    state = reducer(state, { type: 'EXAMINE', hotspotId: 'grand-clock' })
    state = reducer(state, { type: 'SET_CLOCK_TIME', time: '09:23' })
    state = reducer(state, { type: 'MOVE', areaId: 'garden' })

    expect(getVisibleHotspots(state).some((hotspot) => hotspot.id === 'garden-true-light')).toBe(true)

    state = reducer(state, { type: 'OPEN_TRUE_GARDEN_LIGHT' })

    expect(state.screen).toBe('trueFade')
  ...
  it('NORMAL after non-target times does not unlock TRUE route or Garden music', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'ATTACH_CLOCK_HAND' })
    state = reducer(state, { type: 'GO_NORMAL_END' })
    state = reducer(state, { type: 'START_GAME' })
    ...
    expect(reducer(state, { type: 'SET_CLOCK_TIME', time: '15:30' }).flags.trueGardenMusicHeard).not.toBe(true)
  })
})
...
describe('Ending flow', () => {
  ...
  it('true route starts the TRUE END fade without unlocking PHOTO E yet', () => {
    let state = createInitialState()
  ...
  it('TRUE fade can transition to TRUE END and stays unlocked', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'SET_MEMORY_COUNT', count: 4 })
    state = reducer(state, { type: 'UNLOCK_TRUE_ROUTE' })
    state = reducer(state, { type: 'GO_TRUE_END' })
...
describe('Inventory', () => {
  it('obtains, selects, and consumes both clock hands on the grand clock', () => {
    let state = createInitialState()
    state = reducer(state, { type: 'OBTAIN_ITEM', itemId: 'clock-hand' })
    state = reducer(state, { type: 'OBTAIN_ITEM', itemId: 'clock-hour-hand' })
    state = reducer(state, { type: 'SELECT_ITEM', itemId: 'clock-hand' })
    ...
    expect(state.inventory['clock-hour-hand'].obtained).toBe(false)
    expect(state.inventory['clock-hour-hand'].consumed).toBe(true)
  })
})
...
describe('SaveState', () => {
  ...
  it('saves NORMAL at 09:20 and can still trigger Garden music after load', () => {
    const storage = new Map<string, string>()
    vi.stubGlobal('localStorage', {
      getItem: (key: string) => storage.get(key) ?? null,
      setItem: (key: string, value: string) => storage.set(key, value),
    ...
    let loaded = loadGame()
    expect(loaded.inventory.letter.obtained).toBe(true)
    loaded = reducer(loaded, { type: 'SET_CLOCK_TIME', time: trueClockTarget })
    expect(loaded.screen).toBe('game')
    expect(loaded.flags.trueGardenMusicHeard).toBe(true)
    expect(loaded.memories.september23.unlocked).toBe(false)
    vi.unstubAllGlobals()
  })

  it('saves and loads TRUE fade without unlocking the final memory early', () => {
    const storage = new Map<string, string>()
    vi.stubGlobal('localStorage', {
      getItem: (key: string) => storage.get(key) ?? null,
      setItem: (key: string, value: string) => storage.set(key, value),
    ...
    expect(getMemoryCount(loaded)).toBe(4)
    loaded = reducer(loaded, { type: 'SET_CLOCK_TIME', time: trueClockTarget })
    expect(loaded.screen).toBe('trueFade')
    expect(getMemoryCount(loaded)).toBe(4)
    vi.unstubAllGlobals()
  })
